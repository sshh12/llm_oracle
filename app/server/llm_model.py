...
The invalid text will be shown as an alert on the UI to the user.
"""


def validate_question(llm: ChatOpenAI, question: str) -> Tuple[bool, str]:
...
def run_agent(llm: ChatOpenAI, tool_llm: ChatOpenAI, question: str, callback_manager: CallbackManager) -> int:
    event = CustomEvent(
        question,
        None,
    )
    agent = ToolAgentv3(model=llm, tool_model=tool_llm, callback_manager=callback_manager)
